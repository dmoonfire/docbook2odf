...
<xsl:stylesheet
	...
	version="1.0">
  <!-- Templates -->
  <xsl:template match="d:acknowledgments" mode="acknowledgments">
	<!-- Insert the title for the TOC-->
	<xsl:apply-templates select="." mode="acknowledgments.title"/>
	<xsl:apply-templates/>
  </xsl:template>

  <xsl:template match="d:*" mode="acknowledgments.title">
